...
//? Validamos que el ID que nos dan por URL sea valido y en caso tal no lo sea nos rediriga al inicio de admin y así evitar que juegen con nuestra URL
...
//? Read propiertes.
$consultaPropi = "SELECT * FROM propiedades WHERE id = ${id}";
$resultadoPropi = mysqli_query($DB, $consultaPropi);
$propiedad = mysqli_fetch_assoc($resultadoPropi);

//? Array para verificar errores en el formulario.
$errores = [];

//* Re asignamos los valores de la propiedad para evitar que el usuario los re escriba.
...
/*     if(!($imagen["type"] == "image/jpeg" || $imagen["type"] == "image/png")){ //* validamos el tipo de archivo que se va a subir.
...
/*     echo "<pre>";
    ...
    if (empty($errores)) { //* Si el arreglo esta vacio haga:
        ...
        //? Actualizamos imagen:
        if($imagen['name']){
            unlink($carpetaImagenes . $propiedad['imagen']); //* La función UNLINK nos permite eliminar imagenes o archivos de una ruta en especifico. Primer parametro = ubicación del archivo. Segundo parametro = nombre del archivo.
        ...
        //? upload img
            //* Cuando subimos una imagen se almacena de forma temporal en el servidor, pero necesitamos de de una función de php para llevarla a su destino:
       ...
       /*  echo $query; //*  Siempr es bueno verificar los Query.
        exit; */
        ...
        if ($resultado) {  //* Hacemos un if por si es resultado nos avise que se hizo correctamente.
            ...
            header('Location: /admin?mensaje=2'); //* Aplicamos QueryString (URL con parametros). El Query String siempre inicia con un signo de interrogacion (?). con un andperson & podemos agregarle otra variable u otro parametro. 
        }
    }
}
...
<main class="contenedor seccion">
    ...
    <form method="POST" class="formulario" enctype="multipart/form-data"> <!-- Borramos el action para que nos redireccione a la misma URL, respete nuestra validación y siga funcionando correctamente. -->
        ...
        <fieldset>
            ...
            <textarea id="descripcion" name="descripcion" maxlength="100"><?php echo $descripcion ?></textarea>
